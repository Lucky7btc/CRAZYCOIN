...
// Copyright (c) 2011-2013 The CRAZY developers
...
const string strMessageMagic = "CRAZY Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // CRAZY: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::AreInputsStandard(const MapPrevTx& mapInputs) const
{
    ...
    for (unsigned int i = 0; i < vin.size(); i++)
    {
        ...
        if (whichType == TX_SCRIPTHASH)
        {
            ...
            int tmCRAZYpected;
            tmCRAZYpected = ScriptSigArgsExpected(whichType2, vSolutions2);
            if (tmCRAZYpected < 0)
                return false;
            nArgsExpected += tmCRAZYpected;
        }
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // CRAZY: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // CRAZY: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// CRAZY: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
	...
	if (nBits == 469827583) { nSubsidy = 2750 * COIN; }
	return nSubsidy;
}

// CRAZY: miner's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge, int nHeight)
{
	...
	int64 nSubsidy = nRate * nCoinAge * 33 / (365 * 33 + 8) * CENT;
return nSubsidy;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60;
static const int64 nTargetSpacingWorkMax = STAKE_TARGET_SPACING;
...
// CRAZY: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // CRAZY: target change every block
    // CRAZY: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // CRAZY: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // CRAZY: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // CRAZY: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // CRAZY: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // CRAZY: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // CRAZY: track money supply and mint amount info
    ...
    // CRAZY: fees are not collected by miners as in bitcoin
    // CRAZY: fees are destroyed to compensate the entire network
...
// CRAZY: total coin age spent in transaction, in the unit of coin-days.
...
// CRAZY: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // CRAZY: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // CRAZY: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // CRAZY: record proof-of-stake hash value
...
bool CBlock::CheckBlock() const
{
    ...
    // CRAZY: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // CRAZY: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock()
{
    ...
    // CRAZY: check that the block satisfies synchronized checkpoint
    ...
    // CRAZY: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // CRAZY: verify hash target and signature of coinstake tx
    ...
    // CRAZY: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // CRAZY: getblocks may not obtain the ancestor block rejected
    ...
    // CRAZY: if responsible for sync-checkpoint send it
...
// CRAZY: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "CRAZY", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "CRAZY", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // CRAZY: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // CRAZY: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // CRAZY: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // CRAZY: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // CRAZY: if detected invalid checkpoint enter safe mode
    ...
    // CRAZY: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // CRAZY: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // CRAZY: record my external IP reported by peer
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // CRAZY: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // CRAZY: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // CRAZY: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // CRAZY: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // CRAZY: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
