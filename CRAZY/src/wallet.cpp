...
// CRAZY: optional setting to unlock wallet for block minting only;
...
// CRAZY: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // CRAZY: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // CRAZY: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // CRAZY: not avatar mode
                        {
...
// CRAZY: create coin stake transaction
...
// CRAZY: check 'spent' consistency between wallet and txindex
// CRAZY: fix wallet spent state according to txindex
...
// CRAZY: disable transaction (only for coinstake)
